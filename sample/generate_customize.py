...
def main():
    ...
    niter = os.path.basename(".\\save\\omnicontrol_ckpt\\model_humanml3d.pt").replace('model', '').replace('.pt', '')
    ...
    n_frames = 196
    is_using_data = not any([args.text_prompt])     # use dataset when no text prompt is given
